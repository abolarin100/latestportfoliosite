...
import { ChatIcon, SendIcon, CloseIcon, MailIcon, GithubIcon, LinkedinIcon } from "./icons";
...
const GREETING: Message = {
  role: "model",
  content: `Hi! I can answer questions about ${profile.name.split(" ")[0]}'s experience, skills, and projects — ask me anything.`,
};
...
function linkify(text: string): React.ReactNode[] {
  ...
  return parts.map((part, i) => {
    ...
    if (part.match(/^[\w.+-]+@[\w-]+\.[\w.-]+$/)) {
      return (
        <a key={i} href={`mailto:${part}`} className="text-army underline underline-offset-2">
          {part}
        </a>
      );
    }
...
export function Chatbot() {
  ...
  useEffect(() => {
    const sendSummary = () => {
      ...
      const blob = new Blob([JSON.stringify({ messages: messagesRef.current })], {
        type: "text/plain",
      });
  ...
  const send = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/chat`, {
        ...
        body: JSON.stringify({
          message: text,
          // Don't send the canned greeting — the backend's system prompt
          // already establishes who it is; sending it back just wastes tokens.
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between border-b border-line px-4 py-3">
          <div>
            <p className="font-display text-[14px] font-medium text-fg">Ask about Jeremiah</p>
            <p className="font-mono text-[11px] text-fg-muted">Usually replies instantly</p>
          </div>
          <button
      ...
      <button
        ...
        aria-expanded={open}
        className="fixed bottom-5 right-5 z-30 flex h-14 w-14 items-center justify-center rounded-full bg-army text-bg shadow-[0_8px_24px_-6px_rgb(0_0_0_/_0.35)] transition-all duration-200 hover:scale-105 active:scale-95"
      >
